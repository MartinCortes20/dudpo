...
export function HeroExpand() {
  return (
    ...
    >
      ...
      ════════════════════════════════════════ */}
      <section id="about" className="relative py-12 sm:py-20 lg:py-32 overflow-hidden">

        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-10 sm:gap-12 lg:gap-16 lg:min-h-[80vh]">
            ...
            <div className="w-full lg:w-[45%] flex-shrink-0 relative flex justify-center">
              ...
              <motion.div 
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 sm:w-64 sm:h-64 md:w-96 md:h-96 bg-gradient-to-tr from-electric-violet/40 to-electric-magenta/40 rounded-full blur-[80px]"
                animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0.8, 0.5] }}
                transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
              />

              {/* Photo Container */}
              <div className="relative z-10 w-[82%] max-w-[300px] sm:w-full sm:max-w-sm md:max-w-md">
                {/* Liquid Chrome floating blobs around photo */}
                <motion.div 
                  className="absolute -top-8 -right-3 w-14 h-14 sm:-top-10 sm:-right-4 sm:w-20 sm:h-20 bg-gradient-to-br from-white via-chrome-300 to-electric-violet rounded-full blur-[2px] shadow-[0_0_30px_rgba(255,255,255,0.8)] opacity-70 mix-blend-screen"
                  animate={{ y: [0, 20, 0], x: [0, -10, 0], scale: [1, 1.1, 1] }}
                  transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
                />
                <motion.div 
                  className="absolute bottom-10 -left-5 w-12 h-12 sm:-left-8 sm:w-16 sm:h-16 bg-gradient-to-br from-electric-magenta via-white to-electric-violet rounded-[40%_60%_70%_30%] blur-[2px] shadow-[0_0_20px_rgba(217,70,239,0.8)] opacity-60 mix-blend-screen"
            ...
            <div className="w-full lg:w-[55%] flex items-center">
              <div className="space-y-6 sm:space-y-8 w-full max-w-2xl py-0 sm:py-8">
                ...
                <div className="space-y-6">
                  ...
                  <motion.div
                    ...
                    className="inline-flex items-center gap-2 px-4 sm:px-6 py-2 rounded-full bg-gradient-to-r from-chrome-200 via-white to-chrome-300 shadow-[0_0_20px_rgba(255,255,255,0.3),inset_0_2px_5px_rgba(255,255,255,0.8)]"
                  >
                  ...
                  <div className="space-y-4">
                    <h3 className="font-outfit font-medium text-2xl sm:text-3xl md:text-4xl leading-tight text-white">
                    ...
                    <p className="text-chrome-300 text-base sm:text-lg leading-relaxed font-light">
                    ...
                    <div className="flex flex-wrap gap-3 mt-3">
                      {softwareLogos.map((logo) => (
                        <div
                          key={logo.name}
                          title={logo.name}
                          className="w-10 h-10 sm:w-11 sm:h-11 rounded-[14px] overflow-hidden flex items-center justify-center bg-white shadow-lg hover:shadow-[0_0_15px_rgba(255,255,255,0.4)] hover:-translate-y-1 transition-all duration-300"
                        >
                ...
                {/* CTAs */}
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 flex-wrap pt-2 sm:pt-4 [&>*]:w-full sm:[&>*]:w-auto">
...
function PremiumCard({ children, sparkColor, title }: { children: React.ReactNode; sparkColor: "violet" | "magenta"; title: string }) {
  ...
  return (
    <div className={`relative group rounded-3xl p-5 sm:p-6 bg-black/40 backdrop-blur-xl border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] transition-all duration-500 overflow-hidden ${borderHover}`}>
      {/* Animated Gradient Background on Hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      
      {/* Decorative Spark */}
      <div className={`absolute top-5 right-5 sm:top-6 sm:right-6 w-1.5 h-1.5 rounded-full ${glow}`} />
